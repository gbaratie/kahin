...
Les secrets (`DATABASE_URL`, `ADMIN_USERNAME`, `ADMIN_PASSWORD`, `ADMIN_AUTH_SECRET`, et optionnellement `MICROSOFT_CLIENT_ID` / `MICROSOFT_CLIENT_SECRET` / `MICROSOFT_TENANT_ID` / `MICROSOFT_REDIRECT_URI` / `FRONT_ORIGIN`, etc.) s’ajoutent dans l’onglet **Environment** du service Render ; ils ne figurent pas dans `render.yaml` pour éviter de commiter des mots de passe. Voir [microsoft-auth.md](microsoft-auth.md) pour la connexion Microsoft des étudiants.
